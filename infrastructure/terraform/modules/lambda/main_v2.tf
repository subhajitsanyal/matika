...
locals {
  # The v2 Lambdas fetch DB credentials at cold-start from Secrets Manager
  # via DB_SECRET_ARN. PGHOST/PGPASSWORD/etc. no longer appear in the
  # Lambda environment configuration — anyone with
  # `lambda:GetFunctionConfiguration` permission previously could read the
  # password in plaintext, which is a HIPAA / DPDP audit finding.
  db_env = {
