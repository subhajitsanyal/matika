...
| **PT-V2-07** | Text-input fallback (deterministic) | **REGRESSED (F27, 2026-05-10)** | `patient_logging_happy_path.yaml`. The CI gate. Now fails at `matika_text_fallback` because `patient_home_start_conversation` button is disabled by a legacy v1 Mac Mini health check (see F27 — `clearState` wipes `macMiniBaseUrl`, mDNS finds no Mac Mini, health stays OFFLINE, canConverse=false). Unblocks once F27 ships. |
...
| **EDGE-V2-03** | Bedrock Guardrail block | **blocked by F27 (2026-05-10)** | `patient_guardrail_block_text.yaml` taps `patient_home_start_conversation` to reach the conversation screen — same path the CI gate uses (PT-V2-07). F19 (parser crash) was RESOLVED in code 2026-05-10 but cannot be re-verified end-to-end until F27 (legacy Mac Mini health gate disables the conversation button after clearState) ships. Direct lambda invoke against bedrock-router with a guardrail-triggering transcript remains a valid alternate path for F19 verification. |
...
| **EDGE-V2-14** | Network drop during sync of manual log | **PARTIAL (2026-05-10)** | Implicit evidence from Phase-1 PT-V2-15..20: 6 vital saves all eventually landed in S3 over a 9-minute window (some with 4–5min lag), demonstrating WorkManager backoff + retry resilience. The dedicated wifi-cycle harness (`scripts/matika-bp-network-drop.sh` + `edge_v2_14_part{1,2}_*.yaml`) is authored but blocked: `launchApp: clearState: false` between Maestro session boundaries does NOT preserve Cognito auth state when offline — the splash routes back to login because token validation fails. Workarounds documented inline in the orchestrator script. |
