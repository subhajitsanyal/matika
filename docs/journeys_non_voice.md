...
| **CG-V2-07** | Receive threshold breach push | RDS+SQS half **PASS** (via E2E-V2-02); push delivery **blocked on F15** | Alert row + SQS message land correctly post-F11. notification-sender consumes the message but `getCaregiverDeviceEndpoints` returns `[]` because of an unrelated UUID-vs-cognito_sub join + non-existent `endpoint_arn` column (F15). |
| **CG-V2-08** | Receive missed-measurement push | RDS half **PASS** (4 missed_measurement rows already in dev); push delivery **blocked on F15** | check-missed-measurements writes the alert row correctly. notification-sender's missed-measurement path hits the same F15 device-token join. |
| **CG-V2-09** | Receive emergency push | RDS half **PASS** (1 emergency session ended cleanly with `escalations_triggered=["emergency_keyword"]`); push delivery **blocked on F15** | bedrock-router's emergency alert SQS path delivers; same F15 device-token resolution failure on the consumer side. |
...
| **E2E-V2-03** | Missed-measurement alert cycle | RDS half **PASS** (Jane has 4 `missed_measurement` rows in dev `alerts`); push delivery **blocked on F15** | Backend chain (parameter_configs → check-missed-measurements → alerts row + SQS) works. Caregiver-side push is the F15 device-token resolution failure. |
| **E2E-V2-06** | Reminder lapse → patient logs | RDS half **runnable post-F12 fix** (next 18:00–23:59 IST window); push delivery **blocked on F15** | F12 (uppercase 'PATIENT_REMINDER' enum literal) was silently dropping every daily-deadline reminder; fixed and verified live 2026-05-09. The next IST evening window will produce a `patient_reminder` row. Push delivery still blocked on F15. |
