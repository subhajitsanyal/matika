...
| **EDGE-V2-04** | `FORCE_CHANGE_PASSWORD` first-time login | **blocked — Android app gap (2026-05-14, surfaced during Stream C)** | Test harness side ready: `harness_admin_create_force_change_password_user` in `test-automation/scripts/cognito-test-harness.sh` provisions a Cognito user in the FORCE_CHANGE_PASSWORD state. App side is missing — `AuthRepository.signIn()` (android/.../auth/AuthRepository.kt:182) returns `Result.failure("Sign in incomplete: ${result.nextStep}")` when Cognito returns the NEW_PASSWORD_REQUIRED challenge; there is no UI to enter the new password. Adding it is a discrete ~1-day Android work item: capture `nextStep == AuthSignInStep.CONFIRM_SIGN_IN_WITH_NEW_PASSWORD`, route to a new screen, call `Amplify.Auth.confirmSignIn(newPassword)`. Once that lands, EDGE-V2-04 can flip via the existing harness without further test-side work. |
...
| **Cognito-interception-blocked** | 0 | CG-V2-01 + PT-V2-23 PASS via Stream C harness (2026-05-14). EDGE-V2-04 reclassified to Android-app-blocked (NEW_PASSWORD_REQUIRED next-step handling absent in `AuthRepository.kt:182`). |
