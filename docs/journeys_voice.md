...
| CG-V2-04 | **bench-blocked (F39)** — voice path proven; form-submit step disabled | F23 voice entry is now wired (`add_patient_voice_fab` on caregiver dashboard, bounds (644, 1949)). 2026-05-16 post-reboot bench drove turns 1–8 via acoustic `say -v Rishi` → state advanced CREATED → EXTRACTING_PROFILE; Soda captured each turn (chars=29..70, `#handleFinalResult` clean). The LLM tracked name (mis-heard "Menon" → "Maina" at the Soda layer; LLM accepted what STT delivered), age, conditions, no-allergies, doctor across the conversation phase. Flow then transitions to a typed contact-details form ("We'll send the patient their login by email and SMS…"); `patient_credentials_submit` stays `enabled="false"` with valid email + E.164 phone typed in. **Tap does not fire any HTTP request** (API GW 4XX = 0, both `create-patient` lambdas silent, no `patients` row created). Filed as **F39** in `testing_todos_v2.md`. Voice path itself proved end-to-end up to the form transition; original "F23 architecture-blocked" status is now resolved. |
...
- **F39 (NEW, open, 2026-05-16)** — CG-V2-04 voice patient onboarding form-submit step disabled. Voice conversational phase works through turn 8; typed contact-form `patient_credentials_submit` button stays `enabled="false"` and tap fires no HTTP request. Patient never created. Details in `testing_todos_v2.md` F39.
