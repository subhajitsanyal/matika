...
| PT-V2-06 | **blocked by F27 (re-run gated, 2026-05-10)** | F25 (online STT fallback) is RESOLVED, so the bn-IN run that hard-failed earlier today is eligible for re-verification. But the flow taps `patient_home_start_conversation` to reach the conversation screen, and F27 (legacy v1 Mac Mini health check disables that button when mDNS finds no Mac Mini after clearState) now blocks the entry. Once F27 ships, re-run `scripts/matika-voice-run.sh patient_voice_bp_bn_single_turn` with `MATIKA_BN_AUDIO=<path-to-aiff>` exported. |
