...
> **Audience:** the orchestrator / founder planning the path from current state to closed-beta open. **Snapshot date:** 2026-05-18 evening (post `testing-todos-v2-phase7` session — F52 sub-issues 1+2+4 closed; F53 surfaced + closed end-to-end on bench; CG-V2-19/20/21 all PASS; Manage Care Team is now fully exercised on staging. Staging soak clock — INFRA-1 — should be **reset** because this session's terraform apply was a real infra change; next sign-off target shifts forward by however many days the soak was meant to run. Confirm clock impact in the launch tracker before locking T-21.).
>
...
| **F52 sub-issue 1** | `'relative'` enum filter in `invite-doctor` + `remove-team-member` | ✅ DEPLOYED to staging `remove-team-member` 2026-05-18 (CodeSha `mn7s8ynr…`); invite-doctor still code-only (Phase 2). | Resolved for beta scope. | `remove-team-member` also got the v2 vocab cleanup in the same deploy: filter is now `pl.relationship IN ('caregiver','doctor') AND is_primary = false` (there is no `attendant` persona in v2). |
| **F52 sub-issue 2** | UUID-vs-short-code accept in `invite-doctor` + `remove-team-member` | ✅ DEPLOYED to staging `remove-team-member` 2026-05-18 (same deploy as sub-issue 1). Live smoke: short-code patientId → 403 (not 500/22P02). invite-doctor pending Phase 2. | Resolved for beta scope. | Same predicate template as the invite-attendant fix: `(p.patient_id = $1 OR p.id::text = $1)`. |
| **F52 sub-issue 3** | `ses:VerifyEmailIdentity` IAM gap on `carelog-staging-lambda-rds-ses` | DEFERRED | No if EX-3 lands path (b) (SES prod access removes the call path entirely) | Otherwise terraform apply adding the action to the inline policy. Same shape as F49 IAM fold-in. Note: sub-issue 4 below now masks this from the UX even if EX-3 slips. |
| **F52 sub-issue 4** | invite-attendant returns 500 when SES errors despite DB writes committing | ✅ DEPLOYED to staging `invite-attendant` 2026-05-18 (CodeSha `Eafu65C3…`). Android client updated. | Resolved for beta. | Lambda wraps the SES block in its own try/catch; on failure returns 201 with `emailStatus=delivery_failed` + `emailDeliveryError` + fallback message. `InviteAttendantScreen` mounts a warning-icon variant of `InviteSentDialog` (testTag `invite_sent_dialog_delivery_failed`) instead of silently navigating. Live E2E verification deferred to next bench (would need an active SES failure to exercise the path). |
...
| **Voice F23 retry path (CG-V2-04 re-verify on current build)** | PARTIAL 2026-05-18 — turns 1+2 PASS (transcript_history shows both round-trips on staging session `c8763218-...`); turn 3 mic-active trigger never fires (runner timeout 150s; previous session was 90s — extending didn't help). | Root cause is NOT the timeout: `RecognitionListener.onReadyForSpeech` never fires on the 3rd mic tap, so STT never opens. Likely a Compose state mismatch, stuck SpeechRecognizer after 2 listen cycles, or `matika_mic` testTag pointing at a non-active element post-turn-2. Investigation requires app-level instrumentation (broader logcat capture during the failure window + ConversationViewModel state introspection). Punt to a focused voice-debug session; don't re-attempt with another timeout bump. |
| **Manage Care Team — UI care-team rendering** | ✅ **CG-V2-20 PASS 2026-05-18** — `cg_v2_care_team_list.yaml` ran on bench; rendered new caregiver row + Primary badge for inviter. Screenshot evidence at `docs/evidence/cg_v2_care_team_list_pass_20260518.png`. | v2 vocab cleanup landed: `CareTeamScreen.kt` iterates `caregivers` (legacy `attendants`/`relatives` sections dropped). TestTags `care_team_list`, `care_team_member_<id>` + name/email/role/primary/remove sub-tags wired. |
| **Manage Care Team — remove-team-member** | ✅ **CG-V2-21 PASS 2026-05-18** — `cg_v2_care_team_remove.yaml` ran end-to-end via the real API GW path. CloudWatch RequestId `3da7c41a-...`, `persona_links.is_active=false`, `audit_log` row, Cognito disabled. Screenshot at `docs/evidence/cg_v2_care_team_remove_real_path_pass_20260518.png`. | Bench surfaced **F53**: API GW route was a deliberate MOCK stub + `users.deactivated_at` column missing from v2 schema. Both fixed in same session (terraform AWS_PROXY wire + lambda code drops `deactivated_at`). See `testing_todos_v2.md` F53 for the full closure record. |
