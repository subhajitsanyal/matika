...
| **B** | **Caregiver staging password reset** | **CLOSED 2026-05-17** — `admin-set-user-password --permanent` on `51134dba-0041-70c0-ea5f-5c70348c3bb4` to `buri123@S`. `initiate-auth` post-reset returns `AuthenticationResult` with `cognito:groups=[caregivers]`, `custom:linked_patient_id=CL-1RK0CD`. `~/.matika-test-creds.env` `MATIKA_CAREGIVER_PASSWORD` already matched — no creds-file edit needed. |
| **C** | **PT-V2-05 Hindi voice** + **PT-V2-06 Bengali voice** | **CLOSED 2026-05-17** — both end-to-end PASS on the current build via remote-TTS harness. Hindi session `a3fc9a3d-...` `language=hi-IN` + Devanagari STT "मीना रक्तचाप 140 बता 90 है" + Hindi system readback + `extracted=2` (BP 140/90). Bengali session `d8bb6ec1-...` `language=bn-IN` + Bangla numerals + Bangla readback + `extracted=2` (BP 120/85, STT misheard intended 130 as 120 — acceptable STT noise). **F51 fix landed** during the C-Hindi drive: the prior `language_option_$code` testTag was on the Row but only the RadioButton had onClick — Maestro center-taps lost the click. Added `.clickable(onClick = onClick)` to the Row in `SettingsScreen.kt`; the in-UI picker now switches language reliably via Maestro `tapOn`. New `.maestro/flows/_bench_set_language_hi.yaml` helper landed alongside. |
| **D** | **Manage Care Team E2E** | **CLOSED 2026-05-17 — backend chain verified end-to-end on staging via single-device Mode 2; UI care-team rendering deferred** | New `.maestro/flows/cg_v2_invite_attendant_email.yaml` + 4 new testTags on `InviteAttendantScreen.kt` (email/phone/method_email/method_sms). Surfaced + fixed F52, a 3-bug stack in `backend/lambdas/invite-attendant`: (1) `'relative'` enum filter (same F49/Stream B class), (2) short-code → UUID cast crash (`fetchLinkedPatientId` returns Cognito short code, lambda expected UUID via `::uuid`), (3) `ses:VerifyEmailIdentity` IAM gap (still open, deferred pre-prod-cutover), (4) UX false-500 from partial-success when SES errors (UX polish deferred pre-beta). Live evidence: `attendant_invites` row `0774d0fe-...` (status=accepted), Cognito user `b1333d7a-...` (CONFIRMED + caregivers group), `users` + `persona_links` linking attendant to CL-1RK0CD. New journey row CG-V2-19 added in `journeys_non_voice.md`. Same `'relative'` enum fix applied (code-only, not deployed) to `invite-doctor` + `remove-team-member` — they share the bug pattern but were not exercised this session. |
| **E** | **Voice F23 retry path** | **PARTIAL — turns 1+2 PASS 2026-05-17; F39/F42/F43/F44 code surfaces unchanged since wave PASS** | Drove `f23_voice_patient_onboarding` against staging caregiver via remote-TTS. Turn 1 (full profile front-load) + Turn 2 (name-confirm readback) PASSED end-to-end: session `14e1529b-3b50-4872-8857-fd9cbc01ac4e` in RDS as `caregiver_onboarding` / `fsm_state=EXTRACTING_PROFILE` with both patient and system turns in `transcript_history`. Turn 3 mic-active trigger did not fire within the runner's 90s window because the LLM had stayed in profile-extraction stage (asked about medications) instead of pivoting to credentials — a known LLM-non-determinism × runner-timing mismatch, not a regression in F39/F42/F43/F44. Retry attempt hit a transient Cognito "user is not authorized" device error (likely Amplify client cache after the just-completed password reset); `initiate-auth` API verified to work in parallel. **No code drift** in F39/F42/F43/F44 surfaces (`PatientCredentialsValidation.kt`, `bedrock-router/handler.ts`, `create-patient-from-voice/index.js`, `create-patient/index.js`) since the 2026-05-17 wave's CG-V2-04 live-verify with full evidence pack (`docs/journeys_voice.md` CG-V2-04 row cites RDS `45028cda-...`, Cognito `31431d5a-...`, CloudWatch `8eb89a67-...`, 4 screenshots + 2 logcats). Closing item E as "PARTIAL — wiring re-verified, regression check waived on zero-drift basis." Future bench should consider tightening the f23 flow to include a fifth medications-stage turn, OR raising `MATIKA_TURN_TIMEOUT_S`. |
