...
| 2026-05-03 | C | **Phase C scope reduced to protocol-config conversation only** (caregiver demographic capture stays on v1). Backend tweak: `create-patient` Lambda now returns the new patient's `cognito_sub` in the response body alongside `patientId` / `temporary_password` / `email`; redeployed via targeted Terraform apply. Android wiring: `CreatePatientResponse` + `PatientSetupUiState` extended with `cognito_sub` / `patientCognitoSub`; `PatientProfileConfirmationScreen.onPatientCreated` callback now threads the sub forward; `CareLogRoutes.MATIKA_PROTOCOL_CONFIG_CONVERSATION` added (carries `patientCognitoSub` in path); nav-host branches the post-create-patient navigation on `USE_V2_INFERENCE` AND a non-blank sub. `MatikaConversationViewModel` now reads `patientCognitoSub` from `SavedStateHandle` (Hilt populates from nav args) — when present, runs in caregiver mode (`sessionType=caregiver_onboarding`, `actorCognitoSub=currentUser.userId`, `patientCognitoSub` from arg); else falls back to patient_logging defaults. `lastProtocolResult` flow tracks the `protocol` block on the completing turn; `SessionCompleteCard` renders a caregiver-specific summary (parameters configured / topics configured / topics skipped) when `protocolResult` is present. Tests pass (52 v2-area cases); APK builds. **Device test owed.** |
